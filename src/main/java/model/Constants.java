package model;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

public class Constants {
    public static final List<Integer> prize = new ArrayList<>
            (Arrays.asList(0, 0, 0, 5000, 50000, 1500000, 30000000, 2000000000));

    public static final int FIRST_PRIZE = 2000000000;
    public static final int SECOND_PRIZE = 30000000;
    public static final int THIRD_PRIZE = 1500000;
    public static final int FOURTH_PRIZE = 50000;
    public static final int FIFTH_PRIZE = 5000;

    public static final int CORRECT_NUM_SIX = 6;
    public static final int CORRECT_NUM_FIVE = 5;
    public static final int CORRECT_NUM_FOUR = 4;
    public static final int CORRECT_NUM_THREE = 3;
}
